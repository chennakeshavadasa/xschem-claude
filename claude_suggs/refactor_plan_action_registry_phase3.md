...
- [ ] **d1b. (carried from 3c) Represent semaphore-sensitivity in the data model.** Some operations are unsafe during reentrancy and are gated by `if(xctx->semaphore >= 2) break;` in their switch branch. To migrate the 6 deferred sem-first chords (plain `a`/`b`, `s`, `Ctrl+s`/`Ctrl+f`/`Ctrl+r`) — and any sem-gated command key in d2 — the dispatch needs to know a chord/action is "idle-only" and skip it (before `current_input_ctx`, to avoid `waves_selected` side effects) at `sem>=2`. Candidate: an `idle_only` flag on the binding/action, checked at the top of the DEV_KEY dispatch. Once it exists, the deferred 6 migrate cleanly (add over_graph row, delete the waves guard, keep the `if(sem>=2)break;`).
